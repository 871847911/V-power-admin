            ...
            <a-col :md="8" :sm="24">
              <a-form-item label="存储位置">
                <a-select v-model="queryParam.fileLocation" placeholder="请选择存储位置">
                  <a-select-option
                    v-for="(item, index) in fileLocationDictTypeDropDown"
                    :key="index"
                    :value="item.code"
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a-divider
            type="vertical"
            v-if="
              (hasPerm('sysFileInfo:preview') & (record.fileSuffix === 'png') ||
                record.fileSuffix === 'jpeg' ||
                record.fileSuffix === 'jpg' ||
                record.fileSuffix === 'gif' ||
                record.fileSuffix === 'tif' ||
                record.fileSuffix === 'bmp') & hasPerm('sysFileInfo:delete')
            "
          />
          <a
            v-if="
            ...
            "
            @click="$refs.previewForm.preview(record)"
            >预览</a
          >
          <a-divider
            type="vertical"
            v-if="
              (hasPerm('sysFileInfo:preview') & (record.fileSuffix === 'doc') ||
                record.fileSuffix === 'docx' ||
                record.fileSuffix === 'xls' ||
                record.fileSuffix === 'xlsx') & hasPerm('sysFileInfo:delete')
            "
          />
          <a
            v-if="
              hasPerm('sysFileInfo:preview') & (record.fileSuffix === 'doc') ||
                record.fileSuffix === 'docx' ||
                record.fileSuffix === 'xls' ||
                record.fileSuffix === 'xlsx'
            "
            @click="previewMicrosoft(record)"
            >预览</a
          >
        </span>
      </s-table>
