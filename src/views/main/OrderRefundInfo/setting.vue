    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <a-form-item label="充值状态" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
          <a-radio-group v-decorator="['approvalStatus', { rules: [{ required: true, message: '请选择状态！' }] }]">
...
<script>
import { approvalRefund } from '@/api/modular/main/Recharge/rechargeManage'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      validateFields((errors, values) => {
        if (!errors) {
          approvalRefund({ ...this.record, ...values })
            .then(res => {
              if (res.success) {
