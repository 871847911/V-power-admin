    ...
    <a-card :bordered="false">
      ...
      >
        <template class="table-operator" slot="operator" v-if="hasPerm('banner:add')" >
          <a-button type="primary" v-if="hasPerm('banner:add')" icon="plus" @click="$refs.addForm.add()">新增轮播图管理</a-button>
          <a-button type="danger" :disabled="selectedRowKeys.length < 1" v-if="hasPerm('banner:delete')" @click="batchDelete"><a-icon type="delete"/>批量删除</a-button>
          <x-down
            v-if="hasPerm('banner:export')"
            ref="batchExport"
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a-popconfirm v-if="hasPerm('banner:delete')" placement="topRight" title="确认删除？" @confirm="() => singleDelete(record)">
            <a>删除</a>
          </a-popconfirm>
        </span>
      </s-table>
